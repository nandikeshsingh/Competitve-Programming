...
struct segtree
{
  struct data
  {
	...
	void propagate(int node, int L, int R)
	{
		...
		st[node].val += lazy[node]*(R-L+1);//then change the current node
		cLazy[node] = 0;
    	lazy[node] = 0;
	}
	...
	void build(int node, int L, int R)
	{
		if(L==R)
		{
			st[node].val = a[L];//change the value as per the question.
			return;
		}
		int M=(L + R)/2;
		build(node*2, L, M);
...
signed main()
{
    ...
    for(int i=0;i<m;i++)
    {
        int val=ss.query(b[i]).val;
        if(val==0)continue;
        ss.update(b[i],-val);
        int idx=(b[i]+1)%n;
        if(n-1-b[i]<val)ss.update(idx,n-1,1),val-=(n-idx);
        else ss.update(idx,idx+val-1,1),val=0;
        if(val>=n)
        {
            int d=val/n;
            ss.update(0,n-1,d);
        ...
        if(val>0)ss.update(0,val-1,1);
    }
    for(int i=0;i<n;i++)cout<<ss.query(i).val<<" ";
    cout<<endl;
    return 0;
}
