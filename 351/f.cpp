...
struct segtree
{
  struct data
  {
	...
	void build(int node, int L, int R)
	{
		if(L==R)
		{
			st[node].mn += rmp[mp[L]];//change the value as per the question.
			return;
		}
		int M=(L + R)/2;
		build(node*2, L, M);
...
signed main()
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        if(i==0)
        {
            ss.update(mp[a[i]],a[i]);
            st.insert({a[i],i});
        }
        else 
        {
            int cnt=st.order_of_key({a[i],i});
            int tot=ss.query(0LL,mp[a[i]]).mn;
             now+=(cnt*a[i]-tot);
             ss.update(mp[a[i]],a[i]);
             st.insert({a[i],i});
        }
