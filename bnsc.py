...
for i in range(n):
  ...
  if(get_weekday(date[i])=="Friday"):
    ...
    #  f+=1
    #  j+=1
    if(max_price[j]-min_price[i]>=600):
      # error_list.append(i)
...
for index in range(n):
  print(date[front_error_list[index]]," ",date[back_error_list[index]],max_price[back_error_list[index]]-min_price[front_error_list[index]])